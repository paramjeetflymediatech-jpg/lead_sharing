# 🔧 MySQL Workbench Setup - Step by Step Guide

## 📋 **Problem:** MySQL Workbench installed but not showing data
...
Then click the **lightning bolt ⚡ icon** (Execute) or press `Ctrl + Enter`
...
4. Right-click → Start (or Restart)
...
2. Click the **refresh icon** 🔄 (at the top of Navigator)
...
UNION ALL
SELECT 'leads', COUNT(*), CASE WHEN COUNT(*) > 0 THEN '✅' ELSE '❌' END FROM leads
UNION ALL
SELECT 'categories', COUNT(*), CASE WHEN COUNT(*) > 0 THEN '✅' ELSE '❌' END FROM categories
UNION ALL
SELECT 'tradesperson_profiles', COUNT(*), CASE WHEN COUNT(*) > 0 THEN '✅' ELSE '❌' END FROM tradesperson_profiles;
...
│  MySQL Connections                  │
│  ┌────────────────────────────┐    │
│  │ Lead Sharing Database      │ ←── Your connection
│  │ localhost:3306             │    │
│  └────────────────────────────┘    │
└─────────────────────────────────────┘
```
...
```
┌──────────────────────────────┐
│ Select Rows - Limit 1000     │ ←── Click this!
│ Select Rows (Custom)         │
│ ────────────────────────      │
│ Create Table...              │
│ Alter Table...               │
...
1. **Refresh Schema:** Click 🔄 icon in Navigator if you don't see `lead_sharing`
2. **Expand All:** Double-click `lead_sharing` to expand
3. **Keep Connection Open:** Don't close the SQL editor window
4. **Auto-Refresh:** Go to Edit → Preferences → SQL Editor → Check "Auto-refresh"
