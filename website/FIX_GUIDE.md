...
→ Check if MySQL is at: `C:\Program Files\MySQL\MySQL Server 8.0\bin\mysql.exe`

**Password still doesn't work after reset**  
→ Make sure MySQL service restarted: `net stop MySQL80` then `net start MySQL80`

**Dev server still shows error**
→ MUST restart dev server (Ctrl+C then `npm run dev`) after changing `.env`
