...
async function deploySchema() {
    ...
    try {
        ...
        console.log('≡ƒôè Checking current database state...');
        ...
        console.log(`≡ƒöä Executing ${statements.length} SQL statements...\n`);
            ...
            try {
                ...
                if (tableName) {
                    ...
                    if (tableNames.includes(name)) {
                        console.log(`  ΓÅ¡∩╕Å  ${name} (already exists)`);
                        skipped++;
                    } else {
                        console.log(`  Γ£à ${name} (created)`);
                        created++;
                    }
                }
            } catch (err) {
        ...
        // Verify final state
        console.log('≡ƒôè Verifying final database state...');
        ...
        finalTableNames.forEach(table => {
            ...
            console.log(`  ${isNew ? '≡ƒåò' : 'Γ£ô'}  ${table}`);
        });
        ...
        console.log('\nΓ£à Checking required tables...');
        const missingTables = requiredTables.filter(t => !finalTableNames.includes(t));

        if (missingTables.length > 0) {
            console.log('ΓÜá∩╕Å  Missing tables:', missingTables.join(', '));
            console.log('');
            console.log('Please check your schema.sql file and try again.');
        } else {
            console.log('Γ£à All required tables are present!');
        }

        console.log('');
        console.log('==================================================');
        console.log('Γ£à Schema deployment completed successfully!');
        console.log('==================================================');
        console.log('');
        ...
        console.error('\nΓ¥î Deployment failed!');
        console.error('Error:', error.message);
        console.error('');

        if (error.code === 'ER_ACCESS_DENIED_ERROR') {
            console.error('≡ƒÆí Possible solutions:');
