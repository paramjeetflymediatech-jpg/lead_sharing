...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Γ¥î Cannot connect to MySQL!" -ForegroundColor Red
    Write-Host "   Please ensure MySQL is running and password is correct." -ForegroundColor Red
    exit 1
}
Write-Host "Γ£à MySQL connection successful!`n" -ForegroundColor Green
...
if ($dbResult -notmatch "lead_sharing") {
    Write-Host "Γ¥î Database 'lead_sharing' does not exist!" -ForegroundColor Red
    Write-Host "   Creating database..." -ForegroundColor Yellow
    & $mysqlPath -u root -p$password -e "CREATE DATABASE lead_sharing;"
    if ($LASTEXITCODE -eq 0) {
        Write-Host "Γ£à Database created successfully!" -ForegroundColor Green
    } else {
        Write-Host "Γ¥î Failed to create database!" -ForegroundColor Red
        exit 1
    }
} else {
    Write-Host "Γ£à Database exists!`n" -ForegroundColor Green
}
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "Γ£à Admin user entry created!`n" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Cyan
    ...
    Write-Host "Γ¥î Failed to create admin user!" -ForegroundColor Red
    exit 1
}
