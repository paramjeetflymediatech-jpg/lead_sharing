...
export default function RateJobPage() {
  ...
  const fetchUserAndJobDetails = async () => {
    try {
      ...
      // 2. Fetch job details using homeowner API
      const jobRes = await fetch(`/api/homeowner/jobs/${jobId}`, {
        credentials: "include"
      });
      ...
      // 4. Check if already rated
      if (jobData.has_rated || jobData.hasRated) {
        toast.error("You have already rated this job");
        setTimeout(() => router.push('/homeowner/jobs'), 2000);
        return;
      }

      // 5. Check if has hired tradesperson
      if (!jobData.hired_tradesperson_id && !jobData.hiredTradespersonId) {
        toast.error("No tradesperson was hired for this job");
        setTimeout(() => router.push('/homeowner/jobs'), 2000);
        return;
      }
  ...
  const getTradespersonName = () => {
    if (!job) return 'Unknown Tradesperson';
    return job.hired_tradesperson_name || job.hiredTradespersonName || 'Tradesperson';
  };

  const getJobIdString = () => {
    if (!jobId) return '';
    return String(jobId).substring(0, 8);
  };
        ...
        <div className="max-w-2xl mx-auto px-4">
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 mb-6 border border-gray-200 dark:border-zinc-700 shadow-md">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                  {job.subcategory_name || job.subCategory?.name || 'Job'}
                </h3>
                <p className="text-sm text-gray-600 dark:text-zinc-400 mt-1">
                  {tradespersonName}
                </p>
          ...
          <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-800 rounded-2xl p-8 border border-gray-200 dark:border-zinc-700 shadow-lg">
            ...
            {process.env.NODE_ENV === 'development' && (
              <div className="mb-4 p-3 bg-gray-100 dark:bg-zinc-700 rounded text-xs">
                <p>Job ID: {jobId}</p>
                <p>Tradesperson ID: {job.hired_tradesperson_id || job.hiredTradespersonId}</p>
                <p>Status: {job.status}</p>
                <p>Has Rated: {job.has_rated || job.hasRated ? 'Yes' : 'No'}</p>
              </div>
            )}
