...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.success("Welcome back!"); // 2. Success Notification
      ...
      toast.error(err.message); // 3. Error Notification
      setError(err.message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="relative flex min-h-screen w-full flex-col items-center justify-center bg-white px-4 transition-colors dark:bg-[#000000] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Log in to manage your jobs and quotes.</p>
        </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center px-1">
                ...
                <a href="/auth/forgot-password" name="forgot" className="text-[11px] font-bold text-[#155DFC] hover:underline">Forgot?</a>
              </div>
              <input
                required
                type="password"
        ...
        <div className="mt-8 flex flex-col items-center gap-4 text-sm font-medium">
          <p className="text-zinc-500 dark:text-zinc-400">
            Don't have an account?{" "}
            <a href="/auth/register" className="text-[#155DFC] hover:underline underline-offset-4">Create one now</a>
          </p>
          <a href="/" className="text-zinc-400 transition-colors hover:text-black dark:hover:text-white">← Back to homepage</a>
        </div>
      </div>
    </div>
  );
