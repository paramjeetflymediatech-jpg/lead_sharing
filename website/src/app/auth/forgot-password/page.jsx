          ...
          {!submitted ? (
            <>
              <div className="mb-8 text-center">
                ...
                <h1 className="text-2xl font-extrabold text-black dark:text-white">Reset Password</h1>
                <p className="mt-2 text-sm text-zinc-500">Enter your email and we'll send you a recovery link.</p>
              </div>

              {error && (
                <div className="mb-6 rounded-xl bg-red-50 p-4 text-sm text-red-600 dark:bg-red-900/20">{error}</div>
              )}
            ...
            <div className="text-center py-4">
              ...
              <h1 className="text-2xl font-extrabold dark:text-white">Check your email</h1>
              <p className="mt-3 text-sm text-zinc-500">Reset link sent to <span className="font-bold text-black dark:text-white">{email}</span></p>
              <button onClick={() => setSubmitted(false)} className="mt-8 text-sm font-bold text-[#155DFC]">Try another email</button>
            </div>
          )}
        </div>
        <div className="mt-8 text-center">
          <Link href="/auth/login" className="text-sm text-zinc-400 hover:text-black dark:hover:text-white">← Back to Login</Link>
        </div>
      </div>
    </div>
  );
