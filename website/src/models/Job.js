...
const detailedJobMapper = (row) => ({
  ...
  subCategory: row.sub_category_name
    ? { _id: row.sub_category_id, name: row.sub_category_name }
    : row.sub_category_id,
...
export const Job = {
  ...
  async create(data) {
    ...
    const [result] = await pool.query(
      `INSERT INTO jobs (description, homeowner_id, category_id, sub_category_id, budget_min, budget_max, city, postcode, contact_name, contact_email, contact_phone, job_stage, ownership, start_time, status, media, location) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        description,
        homeowner,
  ...
  async deleteOne(query) {
    ...
    try {
      ...
      await pool.query("DELETE FROM tradesperson_ratings WHERE job_id = ?", [
        jobId,
      ]);
