...
export const TradespersonProfile = {
  ...
  // NEW HELPER METHOD: Get profile by user_id (tradesperson_id from ratings)
  ...
  // NEW METHOD: Update rating stats for a tradesperson
  async updateRatingStatsForUser(userId) {
    try {
      ...
      console.log('📊 Calculated stats:', { avgRating, totalRatings });

      // Update the tradesperson_profile
      ...
      const [verify] = await pool.query(
        'SELECT average_rating, total_ratings FROM tradesperson_profiles WHERE user_id = ?',
        [userId]
      );

      if (verify[0]) {
        console.log('✅ Verification - Current profile stats:', {
          average_rating: verify[0].average_rating,
          total_ratings: verify[0].total_ratings
        });
      }
  ...
  async updateRatingStats(userId, averageRating, totalRatings) {
    try {
      ...
      console.log('updateRatingStats: Updated profile for user', userId, '- Rows affected:', result.affectedRows);
