...
async function main() {
    ...
    try {
      ...
      DEFAULT 'PENDING'
    `);
        console.log('Γ£à Status column updated.');
    ...
    `, [4, 9, 'imran', 1, true]);
        ...
        console.log('Γ£à Insert successful. New Lead ID:', result.insertId);

    } catch (error) {
        if (error.code === 'ER_NO_REFERENCED_ROW_2') {
            console.error('Γ¥î Insert Failed: Job ID 4 or Tradesperson ID 9 does not exist.');
        } else {
