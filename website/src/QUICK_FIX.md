# ≡ƒÜÇ QUICK FIX - Run This Script!

## The Problem
`.env` file has empty password ΓåÆ MySQL denies access

## The Solution
Run the all-in-one setup script that will:
Γ£à Reset MySQL root password to `root123`
Γ£à Create database `lead_sharing`  
Γ£à Create all tables
Γ£à Update your `.env` file
Γ£à Verify everything works
...
ΓåÆ You must run PowerShell as Administrator (see Step 1)

**"Execution policy error"**  
ΓåÆ Run first: `Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process`

**Script completes but error persists**
ΓåÆ Make sure you restarted the dev server (Ctrl+C then `npm run dev`)
