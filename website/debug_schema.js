...
async function checkSchema() {
    try {
        ...
        output += leads.map(c => `${c.Field} | ${c.Type} | Null:${c.Null} | Default:${c.Default}`).join('\n') + '\n\n';

        output += '--- MESSAGES TABLE ---\n';
        const [messages] = await db.query('DESCRIBE messages');
        output += messages.map(c => `${c.Field} | ${c.Type} | Null:${c.Null} | Default:${c.Default}`).join('\n') + '\n\n';

        output += '--- TRADESPERSON_PROFILES TABLE ---\n';
        const [profiles] = await db.query('DESCRIBE tradesperson_profiles');
        output += profiles.map(c => `${c.Field} | ${c.Type} | Null:${c.Null} | Default:${c.Default}`).join('\n') + '\n\n';
